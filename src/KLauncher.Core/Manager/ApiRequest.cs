...
public class ApiRequest
{
  ...
  public void RemoveHeader(string key) {
    var exists = _headerDictionary.ContainsKey(key);
    if (exists) _headerDictionary.Remove(key);
  }
  ...
  public ResultData<string> Send(int timeoutSeconds = 10) {
  ...
  /// <summary>
  ///     Sends the request with given retry count. If the request is successful, returns the result. If not, returns the
  ///     last result.
  /// </summary>
  /// <param name="retryCount"></param>
  /// <param name="timeoutSeconds"></param>
  /// <returns></returns>
  public ResultData<string> SendWithRetry(byte retryCount, int timeoutSeconds = 10) {
    if (retryCount is 0 or 1) return Send(timeoutSeconds);
    ResultData<string> lastResult = default;
