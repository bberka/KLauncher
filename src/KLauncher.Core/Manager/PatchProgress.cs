...
public class PatchProgress
{
  ...
  /// <summary>
  ///     Whether to use server and client version check. If false, launcher will check patches by file size and last
  ///     modified date.
  /// </summary>
  ...
  private readonly Stopwatch _stopWatchForSpeedCalculation = new();
  private readonly long _totalBytes;
  private readonly int _totalFileCount;

  private long _downloadedBytes;
  private long _downloadedBytesForSpeedCalculation;
  ...
  public string DownloadInformationText =>
    string.Format(DOWNLOADING_BASE_TEXT,
                  DownloadPercent,
                  _downloadedFileCount,
                  _totalFileCount,
                  DownloadSpeed.ToString("0.0"),
                  _lastByteReceivedFileName);
  //
  ...
  protected internal virtual void CompletedDownload(GameFile fileProgress, double completedInSeconds /*,double speed*/) {
    ...
    if (IsFinished) PatchProgressCompleted?.Invoke();
